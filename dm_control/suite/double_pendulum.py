...
def swingup_sparse(time_limit=_DEFAULT_TIME_LIMIT, random=None,
                   environment_kwargs=None):
  """Returns Pendubot sparse balance."""
  physics = Physics.from_xml_string(*get_model_and_assets())
...
class Balance(base.Task):
  """A Pendubot `Task` to swing up and balance the pole."""
  ...
  def initialize_episode(self, physics):
    ...
    Shoulder and elbow are set to a random position near (pi, 0).

    Args:
      physics: An instance of `Physics`.
    """
    eps = 1e-4
    noise = self.random.uniform(-eps, eps, 2)
    physics.named.data.qpos[
        ['shoulder', 'elbow']] = (np.pi + noise[0], noise[1])
